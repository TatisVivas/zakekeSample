...
export default async function CartPage() {
  ...
  const data = await Promise.all(
    items.map(async (it) => {
      ...
      const designUnitPrice = design?.designUnitPrice || 0;
      const basePrice = product?.basePrice || 0;
      const total = (basePrice + designUnitPrice) * (it.quantity || 1);
      const preview = design?.tempPreviewImageUrl || design?.previewFiles?.[0]?.url || null;
      return { it, product, design, total, preview, basePrice, designUnitPrice };
    })
  );
        ...
        {data.map(({ it, product, preview, total, basePrice, designUnitPrice }) => (
          <div key={it.id} className="border rounded p-4 flex gap-4 items-center">
            ...
            <div className="flex-1">
              ...
              <p className="text-sm">Precio base: {new Intl.NumberFormat("es-CO", { style: "currency", currency: product?.currency || "COP" }).format(basePrice)}</p>
              <p className="text-sm">Diseño: {new Intl.NumberFormat("es-CO", { style: "currency", currency: product?.currency || "COP" }).format(designUnitPrice)}</p>
              <p className="font-semibold mt-1">Total: {new Intl.NumberFormat("es-CO", { style: "currency", currency: product?.currency || "COP" }).format(total)}</p>
              <div className="mt-2">
                <Link href={`/customizer?productid=${encodeURIComponent(it.sku)}&quantity=${it.quantity || 1}${it.designId ? `&designid=${encodeURIComponent(it.designId)}` : ""}`} className="px-3 py-2 bg-black text-white rounded">
                  Editar diseño
                </Link>
              </div>
            </div>
